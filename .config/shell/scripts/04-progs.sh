...
#  - separate x11 and wayland stuff, possibly also gtk/qt

# x11 terminal emulators
for i in aterm ate blackbox contour alacritty
    do command -v $i >/dev/null 2>&1 && export TERMINAL="$i"
done

# wayland terminal emulators
test -n "$WAYLAND_DISPLAY" -o "$XDG_SESSION_TYPE" = "wayland" && for i in ate blackbox contour foot alacritty
    do command -v $i >/dev/null 2>&1 && export TERMINAL="$i"
done
...
test -n "${WAYLAND_DISPLAY:-$DISPLAY}" && for i in rage-player ffplay vlc mplayer smplayer qmplay2 celluloid mpv
...
test -n "${WAYLAND_DISPLAY:-$DISPLAY}" && for i in gv sioyek atril xreader qpdfview lumina-pdf xpdf epdfview katarakt mupdf apvlv zathura
...
test -n "${WAYLAND_DISPLAY:-$DISPLAY}" -a -n "$XDG_CURRENT_DESKTOP" && for i in nautilus caja rox xfe pcmanfm thunar nemo krusader dolphin qtfm
